...
@section('body')
    <div class="flex-1 flex flex-col px-4 md:px-12">
        <div class="content-wrapper" data-resource-catalog data-page-size="8">
            ...
            {{-- ========== 分類分頁 + 搜尋：文件 / 簡報 / Campaign Kit ========== --}}
            <div class="self-stretch flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div class="inline-flex items-center gap-2 flex-wrap" role="tablist" aria-label="資源分類">
                ...
                <label class="search-field w-full md:w-56 shrink-0 px-3 py-2.5 bg-white rounded-[10px] border border-gray1 inline-flex items-center gap-2">
                    ...
                    <span class="sr-only">搜尋資源名稱</span>
                    <input type="search" placeholder="搜尋資源名稱" class="w-full min-w-0 bg-transparent text-cb3 text-gray5 placeholder:text-gray3 outline-none" data-catalog-search>
                    ...
                    <div class="self-stretch overflow-x-auto">
                        <div class="min-w-[720px]">
                            ...
                            <div class="px-4 py-3 bg-bg rounded-t-[10px] inline-flex w-full items-center">
                                <div class="w-32 text-cb3 text-gray4">更新日期</div>
                            ...
                            {{-- 資料列（資料：config marketingResources） --}}
                            <div class="flex flex-col">
                                @foreach ($page->marketingResources as $item)
                                    ...
                                    >
                                        ...
                                        <div class="w-36">
                                            <span class="text-cb3 text-brand2">{{ $category['label'] }}</span>
                                        </div>
                                        <div class="flex-1 pr-4 text-cb3 text-gray5">{{ $item['title'] }}</div>
                                        <div class="w-20 text-eb3 text-gray4 uppercase">{{ $item['format'] }}</div>
                                        <div class="w-24">
